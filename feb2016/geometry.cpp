...
void Shape::fix_rect(const Rect &rect, vector2d *v)
{
	fix_rect_x(rect, v);
	fix_rect_y(rect, v);
}
...
Circle::Circle(const vector2d &c, double r)
	:m_x(c[0]), m_y(c[1]),
	 m_r(r)
{
}
...
const Shape &Circle::operator+=(const vector2d &rhs)
{
	m_x += rhs[0];
	m_y += rhs[1];
	return *this;
...
const Shape &Line::operator+=(const vector2d &rhs)
{
	m_x1 += rhs[0];
	m_y1 += rhs[1];
	m_x2 += rhs[0];
...
const Shape &Rect::operator+=(const vector2d &rhs)
{
	m_x += rhs[0];
	m_y += rhs[1];
	return *this;
